...
static void invertcursor(struct tty *tp)
{
	...
	xorcursor(G.vt.pos.row,
	          ...
	          G.lcd.grayplanes[0] ? : (void*)LCD_MEM);
}
...
static void drawgl(struct glyph *glyph, int row, int col)
{
	if (G.vt.gr & GR_INVERSE)
		drawglyphinv(glyph, row, col, G.lcd.grayplanes[0] ? : (void*)LCD_MEM);
	else
		drawglyph(glyph, row, col, G.lcd.grayplanes[0] ? : (void*)LCD_MEM);
}
...
static void clearrows(int top, int bottom)
{
	int n = bottom - top + 1;
	if (n <= 0) return;
	memset((G.lcd.grayplanes[0] ? : (void*)LCD_MEM)+180*top, -!!(G.vt.gr & GR_INVERSE), 180*n);
	if (G.lcd.grayplanes[1])
